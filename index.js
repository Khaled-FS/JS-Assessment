...
function getMenuItemsByIngredient(menu, ingredient) {
  let storage = [];
  let result = menu.filter((ingredientt) => {
    if (ingredientt.ingredients.includes(ingredient)) {
      return storage.push(ingredientt);
    } else {
      return false;
    }
  });
